...
class GenerateDecoderOnlyOutput(ModelOutput):
    ...
    # Add fields for speculative decoding stats
    assistant_generated_tokens: Optional[int] = None
    main_model_generated_tokens: Optional[int] = None
    assistant_total_generated_tokens: Optional[int] = None
...
class GenerationMixin:
    ...
    def _assisted_decoding(
        ...
        **sample** (depending on `do_sample`), assisted by candidate sequences. Assisted generation is an example of a
        ...
        # Modification: Initialize counters for token statistics
        self.assistant_total_generated_tokens = 0 # Tokens generated by the assistant model
        self.assistant_generated_tokens = 0  # Tokens accepted from the assistant model
        self.main_model_generated_tokens = 0  # Tokens generated by the main model
        ...
        while self._has_unfinished_sequences(this_peer_finished, synced_gpus, device=input_ids.device):
            ...
            self.assistant_total_generated_tokens += candidate_input_ids.size(1) - input_ids.size(1)
            ...
            # Modification: Update counters for token statistics
            self.assistant_generated_tokens += n_matches.item()
            self.main_model_generated_tokens += 1
        ...
        if return_dict_in_generate:
            if self.config.is_encoder_decoder:
                return GenerateEncoderDecoderOutput(
                    ...
                    decoder_hidden_states=decoder_hidden_states,
                    past_key_values=model_kwargs.get("past_key_values"),
                    # speculative decoder performance stats
                    assistant_generated_tokens=self.assistant_generated_tokens,
                    main_model_generated_tokens=self.main_model_generated_tokens,
                    assistant_total_generated_tokens=self.assistant_total_generated_tokens,
                )
            else:
                return GenerateDecoderOnlyOutput(
                    sequences=input_ids,
